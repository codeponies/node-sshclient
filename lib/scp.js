 ...
 */
SCP.prototype.upload = function(localFiles, remotePath) {
	var proc;
  var self = this;

  return new Promise((resolve, reject) => {
	  proc = self.uploadProc(localFiles, remotePath);
  	self.listener.listenProc(proc, (result) => {
      resolve(result);
    });
  });
 ...
 */
SCP.prototype.download = function(remoteFiles, localPath) {
	var proc;
  var self = this;

  return new Promise((resolve, reject) => {
    proc = self.downloadProc(remoteFiles, localPath);
  	self.listener.listenProc(proc, (result) => {
      resolve(result);
    });
  });
